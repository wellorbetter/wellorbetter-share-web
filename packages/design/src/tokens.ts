...
:root {
  ...
  /* ── 形状 ──────────────────────────────────────────────────────
     ...
     --shape-pill 和 --shape-round 数值相同但语义不同，拆开是为了让
     按钮的去胶囊化变成改一行的事：
       --shape-round  几何约束，不是审美选择。用在 6px 高的进度条和
                      填充上，半径必须大于高度的一半才能收成真正的
                      半圆端帽。这个值不该动。
       --shape-pill   审美选择。按钮、标签、导航项、徽章。想收敛成
                      更克制的圆角，只改这一个值，不会碰到上面那些。

     （早先这里写着「改小会把头像切成方块」——那是错的。头像用的是
     硬编码 border-radius: 50%，从来没走过这个 token。） */
  --radius: 8px;
  --radius-sm: 5px;
  --radius-xs: 3px;
  --shape-pill: 999px;
