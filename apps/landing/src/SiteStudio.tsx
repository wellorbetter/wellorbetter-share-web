...
const quickActions = {
  en: ["Make it more minimal", "Make it dark and technical", "Focus on open source", "Focus on AI / agent work", "Make the copy shorter"],
  zh: ["更极简一点", "改成深色、技术感更强", "重点展示开源贡献", "重点展示 AI / Agent 项目", "文案再短一点"],
} as const;
...
export default function SiteStudio({ username }: { username: string }) {
  ...
  useEffect(() => {
    ...
    generateSite(username, initialIntent, locale, controller.signal)
      .then((result) => {
          ...
          try {
            ...
            if (isSiteSpec(saved) && saved.owner.toLowerCase() === result.portfolio.profile.login.toLowerCase()) {
              ...
              if (!validateSiteSpec(repaired, result.portfolio).length) {
                setSpec(repaired);
                setMessages([{ role: "agent", text: locale === "zh" ? "我恢复了你上次保存在这台设备上的草稿。继续告诉我怎么改。" : "I restored the draft saved on this device. Tell me what to change next." }]);
                return;
              }
            }
        ...
        setSpec(result.spec);
        setMessages([{ role: "agent", text: result.agent.mode === "ai" ? (locale === "zh" ? "我读完 GitHub 了，已经先帮你做了一版。你可以直接用自然语言让我改。" : "I read the GitHub profile and built a first version. Tell me what to change in plain language.") : (locale === "zh" ? "基础版本已经生成。当前服务端没有启用付费模型，但常用编辑仍然可以直接做；配置模型后会自动升级成 AI Agent。" : "The baseline is ready. Paid AI is not enabled on the server, but common edits still work; once a model is configured this upgrades automatically.") }]);
      })
  ...
  async function regenerate() {
    ...
    try {
      ...
      if (spec) setHistory((items) => [...items.slice(-19), spec]);
      setGeneration(result); setSpec(result.spec);
      setMessages((items) => [...items, { role: "agent", text: locale === "zh" ? `我按新的目标重新生成了页面：${intent || "通用个人主页"}` : `I regenerated the site for: ${intent || "general personal site"}` }]);
  ...
  function undo() {
    ...
    setHistory((items) => items.slice(0, -1)); setSpec(previous); setNotice(locale === "zh" ? "已撤销上一次修改" : "Undid the last change");
  }

  async function copyShare() {
    if (!shareUrl) return;
    try { await navigator.clipboard.writeText(shareUrl); setNotice(locale === "zh" ? "草稿分享链接已复制" : "Draft share link copied"); }
    catch { window.prompt(locale === "zh" ? "复制这个链接" : "Copy this link", shareUrl); }
  }

  function resetDraft() {
    if (!generation) return;
    if (spec) setHistory((items) => [...items.slice(-19), spec]);
    localStorage.removeItem(storageKey); setSpec(generation.spec);
    setMessages((items) => [...items, { role: "agent", text: locale === "zh" ? "已恢复 Agent 最初生成的版本。" : "Restored the agent's original version." }]);
  }
